  ...
  return (
    <Link href={`/markets/${m.address}`} className="block h-full">
      <GlareCard className="h-full">
        <div className="flex h-full flex-col rounded-2xl border border-white/8 bg-card p-4 transition-colors duration-200 hover:border-blue-500/25 hover:bg-card/80">
          {/* Header */}
          ...
          {/* Question */}
          <h3 className="mt-3 line-clamp-3 min-h-[60px] font-display text-[15px] font-bold leading-snug tracking-tight text-foreground">
            {m.question}
          </h3>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-white/5 pt-3 text-xs text-muted-foreground">
            <span className="inline-flex items-center gap-1">
              ...
              {m.traderCount > 0
                ? `${m.traderCount} trader${m.traderCount > 1 ? "s" : ""}`
                : "No bets yet"}
            </span>
            {isOpen ? (
              <Countdown
              ...
              <span className="font-semibold text-amber-500">Awaiting result</span>
            )}
          </div>
        </div>
      </GlareCard>
